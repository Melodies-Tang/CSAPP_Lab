...
int isAsciiDigit(int x) {
  int pre = !(((x >> 4) << 4) ^ 0x30);  // if prefix 0x00000030
  return pre & ((!(0x8 & x)) | !((0x4 & x) >> 2 | (0x2 & x) >> 1));
}
/* 
...
int conditional(int x, int y, int z) {
  int con = !!x;  // 1 if x != 0 -> outy
  int outy = ~con + 1;  // 0 -> 0, 1 -> ff
  return (outy & y) | (~outy & z);
}
/* 
